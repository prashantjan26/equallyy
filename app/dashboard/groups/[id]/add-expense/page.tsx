...
export default function AddExpensePage() {
    ...
    const [paidBy, setPaidBy] = useState<string>('') // Store member.id
    ...
    async function fetchData() {
        ...
        if (membersData) {
            ...
            if (user) {
                const isMember = membersData.find(m => m.user_id === user.id)
                if (isMember) {
                    setPaidBy(isMember.id)
    ...
    async function handleSave(e: React.FormEvent) {
        ...
        if (expenseError || !expense) {
            setError(expenseError?.message || "Failed to add expense")
            setSaving(false)
            return
        }
        ...
        if (splitType === 'equal') {
            ...
            splitsToInsert = members.map(member => ({
                ...
                is_settled: false
            })).filter(split => split.amount_owed > 0) // Only insert splits > 0 so DB isn't bloated
        }
        ...
        if (splitsError) {
            setError(splitsError.message || "Failed to split expense")
            setSaving(false)
            return
        }
